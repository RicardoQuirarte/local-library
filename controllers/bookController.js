...
exports.book_delete_post = asyncHandler(async (req, res, next) => {
  // Get details of book and all their book instances (in parallel)
  const [book, allBooksInstances] = await Promise.all([
    Book.findById(req.params.id).exec(),
    BookInstance.find({ book: req.params.id }).exec(),
  ]);

  if (allBooksInstances.length > 0) {
    // Book has books instances. Render in same way as for GET route.
    res.render("book_delete", {
      title: "Delete Book",
      book: book,
    ...
    await Book.findByIdAndDelete(req.body.authorid);
    res.redirect("/catalog/books");
  }
});
